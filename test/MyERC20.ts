...
describe("MyERC20", function () {
  ...
  describe("Deployment", function () {
    it("Should set the right owner", async function () {
      expect(await myERC20.hasRole(await myERC20.ADMIN_ROLE(), owner.address)).to.equal(true);
    });
  ...
  describe("Roles", function () {
    it("Should allow admin to change minter", async function () {
      ...
      expect(await myERC20.hasRole(await myERC20.MINTER_ROLE(), addr1.address)).to.equal(true);
    });

    it("Should allow admin to change burner", async function () {
      await myERC20.changeBurner(addr1.address);
      expect(await myERC20.hasRole(await myERC20.BURNER_ROLE(), addr1.address)).to.equal(true);
    });

    it("Should not allow non-admin to change roles", async function () {
      await expect(myERC20.connect(addr1).changeMinter(addr2.address)).to.be.reverted;
      await expect(myERC20.connect(addr1).changeBurner(addr2.address)).to.be.reverted;
    });
  });
  ...
  describe("Minting", function () {
    it("Should allow minter to mint tokens", async function () {
      ...
      expect(await myERC20.balanceOf(addr2.address)).to.equal(MINT_AMOUNT);
    });

    it("Should not allow non-minter to mint tokens", async function () {
      await expect(myERC20.mint(addr2.address, MINT_AMOUNT)).to.be.reverted;
      await expect(myERC20.connect(addr2).mint(addr2.address, MINT_AMOUNT)).to.be.reverted;
    });
  });
  ...
  describe("Burning", function () {
    it("Should allow burner to burn tokens", async function () {
      ...
      expect(await myERC20.balanceOf(owner.address)).to.equal(INITIAL_SUPPLY - BURN_AMOUNT);
    });

    it("Should not allow non-burner to burn tokens", async function () {
      await expect(myERC20.burn(owner.address, BURN_AMOUNT)).to.be.reverted;
      await expect(myERC20.connect(addr2).burn(owner.address, BURN_AMOUNT)).to.be.reverted;
    });

    it("Should revert if burner tries to burn more than the balance", async function () {
      await myERC20.changeBurner(addr1.address);
      await expect(myERC20.connect(addr1).burn(owner.address, INITIAL_SUPPLY + ethers.parseEther("1"))).to.be.reverted;
    });
  });
  ...
  describe("Permit", function () {
    ...
    const signature = await owner.signTypedData(domain, types, message);      
      ...
      await myERC20.permit(ownerAddress, spender, amount, deadline, v, r, s);

      expect(await myERC20.allowance(ownerAddress, spender)).to.equal(amount);
    });
      ...
      it("Should revert with invalid signature", async function () {
        const ownerAddress = owner.address;
          ...
          await expect(
              myERC20.permit(incorrectOwnerAddress, spender, amount, deadline, v, r, s)
    ...
    it("Should revert if permit expired", async function () {
      ...
      await expect(
          myERC20.permit(ownerAddress, spender, amount, Math.floor(Date.now() / 1000) - 3600, v, r, s)
      ).to.be.revertedWithCustomError(myERC20, "EIP2612PermisssionExpired");
    });
  });
});